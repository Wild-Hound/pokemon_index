...
const Home: NextPage = () => {
  ...
  useEffect(() => {
    if (searchText.length > 2) {
      const matchedResults = pokemonsData?.results?.filter((data) => {
    ...
    return (
      <div className="flex items-center mx-auto mt-10 border-2 rounded border-gray-300 relative">
        <input
          ...
          onFocus={(e) => inputEvent(true)}
          onBlur={(e) => inputEvent(false)}
          onChange={(e) => setSearchText(e.target.value)}
        />
        <button className="py-1 px-3 text-xl text-gray-400 transition duration-300 hover:bg-blue-100">
